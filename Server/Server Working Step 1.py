...
def handle_client(conn, addr, player_id):
    ...
    conn.sendall(f"ASSIGN_ID {player_id}".encode())  # Send the assigned player ID to the client
    try:
        while True:
...
def handle_game_action(player_id, action):
    global current_turn
    # This function handles the game actions like playing a card or drawing a card
...
def start_game():
    ...
    broadcast_game_state()
    for conn in clients:
        conn.sendall("START".encode())  # Send the START message to all clients
