...
def handle_game_action(player_id, action):
    ...
    if action.startswith("PLAY"):
        ...
        print(f"It's a valid play: {is_valid_play(player_hands[player_id], card)}")
        if is_valid_play(player_hands[player_id], card):
            print("Valid play.")
            player_hands[player_id].remove(card)
            discard_pile.append(card)
            print(f"Draw 4 card: {'Draw 4' in card}")
            print(f"Wild card: {'Wild' in card}")
            if "Reverse" in card:
                direction *= -1
            if "Draw 4" in card or "Wild" in card:
                print("Player played a special card.")
                send_message_to_player(player_id, "CHOOSE_COLOR\n")
                # return  # Wait for color choice before proceeding
            if "Draw 4" in card:
                print("Player played a Draw 4 card.")
                print(f"Current Player ID: {current_turn}")
                print(f"Next Player ID: {(current_turn + direction) % len(clients)}")
...
def is_valid_play(hand, card):
    ...
    print(f"Card to play: {card}")
    top_color, top_value = top_card.split(maxsplit=1)
    card_color, card_value = card.split(maxsplit=1)
    if "Wild" in card or "Draw 4" in card:
        return True
    return top_color == card_color or top_value == card_value or card_value in ["Wild", "Draw 4"]
