...
uno_button_rect = pygame.Rect(1720, 900, 180, 50)
uno_pressed = False
...
def draw_game():
    ...
    if your_turn:
        ...
        if len(player_hand) == 2 and any(can_play_card(card, discard_pile) for card in player_hand):
            pygame.draw.rect(screen, (255, 0, 0), uno_button_rect)
            uno_text = font.render("UNO!", True, (255, 255, 255))
            uno_text_rect = uno_text.get_rect(center=uno_button_rect.center)
