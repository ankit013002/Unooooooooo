...
# Pygame setup
pygame.init()
screen = pygame.display.set_mode((1440, 900))  # Changed resolution to 1440x900
...
lobby_background_image = pygame.transform.scale(lobby_background_image, (1440, 900))  # Adjusted to new resolution

game_background_image = pygame.image.load(resource_path(os.path.join('Additional_Assets', 'UNO Refined Game.png')))
game_background_image = pygame.transform.scale(game_background_image, (1440, 900))  # Adjusted to new resolution
...
uno_button_image = pygame.transform.scale(uno_button_image, (240, 80))  # Adjusted button size for new resolution
uno_button_rect = uno_button_image.get_rect(topleft=(1140, 800))  # Adjusted position for new resolution
...
def draw_lobby():
    ...
    screen.blit(connected_text, (80, 40))  # Adjusted position for new resolution
    
    # Adjusted position for "I'm Ready" button
    button_color = (0, 255, 0) if ready else (255, 0, 0)
    button_rect = pygame.Rect(1240, 840, 160, 50)  # Adjusted position for new resolution
    ...
    # Credits button
    credits_button_rect = pygame.Rect(40, 840, 160, 50)  # Adjusted position for new resolution
...
def draw_credits():
    ...
    paragraph_y_offset = screen.get_height() - 160  # Adjusted for new resolution
    color_index = 0
    for word in thank_you_lines:
        ...
        if paragraph_x_offset > screen.get_width() - 160:  # Adjusted wrapping point for new resolution
            paragraph_x_offset = screen.get_width() // 2
            paragraph_y_offset += 40
        color_index += 1

    # Back button
    back_button_rect = pygame.Rect(1280, 10, 140, 50)  # Adjusted position for new resolution
...
def draw_game():
    ...
    for idx in range(connected_players - 1):
        ...
        for i in range(hand_length):
            screen.blit(CARD_IMAGES["Uno Back"], (40 + i * 80, 80 + idx * 60))  # Adjusted for new resolution
        pygame.draw.circle(screen, (255, 255, 255), (20, 100 + idx * 60), 20)  # Adjusted for new resolution
        count_text = circle_font.render(str(hand_length), True, (0, 0, 0))
        count_rect = count_text.get_rect(center=(20, 100 + idx * 60))  # Adjusted for new resolution
    ...
    for i in range(start_index, end_index):
        ...
        if card in CARD_IMAGES:
            screen.blit(CARD_IMAGES[card], (card_x_position, 680))  # Adjusted for new resolution
        else:
            card_text = font.render(card, True, (255, 255, 255))
            screen.blit(card_text, (card_x_position, 680))  # Adjusted for new resolution

    pygame.draw.circle(screen, (255, 255, 255), (20, 680), 20)  # Adjusted for new resolution
    count_text = circle_font.render(str(len(player_hand)), True, (0, 0, 0))
    count_rect = count_text.get_rect(center=(20, 680))  # Adjusted for new resolution
    screen.blit(count_text, count_rect)

    if your_turn:
        turn_text = font.render("Your Turn", True, (255, 255, 255))
        screen.blit(turn_text, (40, 320))  # Adjusted for new resolution
    ...
    if choose_color:
        ...
        for i, color in enumerate(color_buttons):
            ...
            button_rect = pygame.Rect(600 + i * 80, 120, 80, 40)  # Adjusted for new resolution
            pygame.draw.rect(screen, button_color, button_rect)
    ...
    # Draw arrow buttons
    left_arrow_rect = left_arrow_image.get_rect(topleft=(10, 600))  # Adjusted for new resolution
    right_arrow_rect = right_arrow_image.get_rect(topright=(screen.get_width() - 70, 600))  # Adjusted for new resolution
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if show_credits:
                    ...
                    if 1240 <= x <= 1400 and 840 <= y <= 890:
                        if not ready:
                            client_socket.sendall("READY".encode())
                            ready = True
                    elif 40 <= x <= 200 and 840 <= y <= 890:
                        show_credits = True
                elif choose_color:
                    if 600 <= x <= 920 and 120 <= y <= 160:
                        color_index = (x - 600) // 80
                    ...
                    for i, card in enumerate(player_hand):
                        ...
                        if card_start_x <= x <= card_end_x and 680 <= y <= 760:  # Adjusted y-coordinate to match card height
