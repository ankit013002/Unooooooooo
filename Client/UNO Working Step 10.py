...
def listen_to_server():
    ...
    try:
        while True:
            ...
            if not data:
                continue
            # print(f"Received data: {data}")  # Debug received data
            ...
            if data.startswith("CONNECTED"):
                ...
                # print(f"Received state message: {data}")
                if player_id is not None:
                    print("Parsing state message...")
                    parts = data.split(" ")
                    # print(f"Parts: {parts}")
                    if "Wild" in parts[1] or "Draw 4" in parts[1]:
                        print(f"Special card in discard pile: {parts[1]}")
